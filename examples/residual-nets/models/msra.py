...
_netspec = _ds.NetSpecification([[100, 3, 32, 32], [100,]],
                                inputs=['data', 'labels'],
                                predict_inputs=['data'],
                                predict_input_shapes=[[100, 3, 32, 32]])
...
def ResBlock(n_in, in_name, n_out, stride=1):
    ...
    layers.append(_ds.BatchNormLayer(name='resblock{}_bn1'.format(_l_idx)))
...
def _construct_resnet(blocks_per_part):
    ...
    _layers.append(_ds.ConvolutionLayer(
        ...
        Convolution_weight_filler=_ds.PROTODETAIL.FillerParameter(
            type='msra')))
    _layers.append(_ds.BatchNormLayer(name='bn_initial'))
